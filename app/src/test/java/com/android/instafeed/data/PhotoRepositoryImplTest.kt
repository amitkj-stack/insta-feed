...
class PhotoRepositoryImplTest {
    ...
    fun whenNetworkIsAvailableButStateIsNotOkShouldReturnError() = runBlocking {
        val spy = spyk(repository)
        val endpoint = Gson().fromJson("{\"photos\":{\"page\":1,\"pages\":37,\"perpage\":20,\"total\":728,\"photo\":[]},\"stat\":\"false\"}", ApiEndPoint::class.java)
        coEvery { apiService.search(any(), any(), any(), any()).await() } returns Response.success(endpoint)
    ...
    fun whenRetrofitCallTimesOutShouldReturnError() = runBlocking {
        coEvery { apiService.search(any(), any(), any(), any()).await() } throws SocketTimeoutException()
